...
void handleButtons()
{
    ...
    if (runtimeIsNightLight())
    {
            ...
            !nightLightSleepHandled)
        {
            ...
            postEventFromTask(makeSleepRequestedEvent(RequestedSleepKind::NightLightTimeout));
#else
            enterDeepSleep();
#endif
        }
             ...
             buttons[0].pressStart > 0 && buttons[1].pressStart > 0)
    {
        ...
        if (now - earliest >= LONG_PRESS_MS)
        {
            ...
            if (f) f.close();
            if (!written)
                LOGE("[SYNC] Failed to write sync flag\n");
            LOGC("[SYNC] Sync flag written=%d\n", written);
            ESP.restart();
        }
    }
