    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if process.returncode != 0:
                output = "\n".join(last_lines[-10:])
                logger.error(f"yt-dlp failed (exit {process.returncode}):\n{output}")
                raise Exception(f"yt-dlp error (exit {process.returncode}): {last_lines[-1] if last_lines else 'no output'}")
            ...
            youtube_tasks[task_id]['progress'] = 80
            youtube_tasks[task_id]['message'] = 'Zapisywanie pliku...'

            import shutil
            shutil.move(downloaded_file, str(final_path))
...
@router.get("/tracks/youtube/title")
async def get_youtube_title(url: str):
    """Pobiera tytuł filmu z YouTube."""
    try:
        result = subprocess.run(
            ["yt-dlp", "--get-title", "--no-playlist", "--remote-components", "ejs:github", url],
...
def update_track(track_id: int, title: str = Form(...), db: Session = Depends(get_db)):
    """Aktualizuje tytuł utworu."""
